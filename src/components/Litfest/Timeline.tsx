...
const Timline = ({ events }: props) => {
  ...
  return (
    <VerticalTimeline
      className="mx-auto"
      animate={screenWidth > 760 ? true : false}
    >
      {events?.map(function (item: any, index: number) {
        return (
          ...
          >
            <div
              id={`${item.title}`}
              className="flex-col m-0 gap-y-11 cursor-pointer md:mx-2 rounded-md"
            >
              ...
              <div
                className={
                  "flex flex-col w-full " + libre_caslon_text.className
                }
              >
                ...
                {item.link && (
                  <a
                    href={item.link}
                    className="text-center align-center px-4 mx-auto my-2 py-2 rounded-md bg-[#F8F3ED] dark:bg-[#603726] dark:text-white hover:shadow-lg active:scale-90 transition-transform duration ease-out"
                  >
                    Register here
                  </a>
                )}
